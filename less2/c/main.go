...
type solveFunc func(intervals []interval) float64
...
func solve(intervals []interval) float64 {
	n := len(intervals)
	nodes := make([]node, 0, (n+1)*2)

	for i := 0; i < n; i++ {
		nodes = append(nodes, node{intervals[i].Begin, i, NodeBegin})
		nodes = append(nodes, node{intervals[i].End, i, NodeEnd})
	}
	...
	slices.SortFunc(nodes, func(a, b node) int {
		if a.T < b.T {
			...
			return 1
		} else if a.Type == NodeEnd && b.Type == NodeBegin {
			return -1
		} else if a.Type == NodeBegin && b.Type == NodeEnd {
			return 1
		}
		return 0
	})
	...
	cumWeight := make([]float64, n) // cumulative weight
	maxWeight := 0.0                // max cumulative weight
	...
	for _, node := range nodes {
		...
		switch node.Type {
		case NodeBegin:
			w := prevMaxWeight + intervals[idx].Weight
			cumWeight[idx] = w
			maxWeight = max(maxWeight, w)
		case NodeEnd:
			prevMaxWeight = max(prevMaxWeight, cumWeight[idx])
		}
	}

	if debugEnable {
		log.Println("weight:", cumWeight)
	}
...
func run(in io.Reader, out io.Writer, solve solveFunc) {
	...
	for i := 0; i < n; i++ {
		b, e, w, err := scanThreeFloat(sc)
		if err != nil && err != io.EOF {
			panic(err)
		}
		intervals = append(intervals, interval{b, e, w})
	}

...
type solveFunc func(way []string) int

func solve(way []string) int {
	n := len(way)
	res := makeMatrix[int](n+1, 3)
	...
	for i := 0; i < n; i++ {
		for j, c := range way[i] {
			...
			switch j {
			case 0:
				res[i+1][0] = max(res[i+1][0], v)
				res[i+1][1] = max(res[i+1][1], v)
			case 1:
				res[i+1][0] = max(res[i+1][0], v)
				res[i+1][1] = max(res[i+1][1], v)
				res[i+1][2] = max(res[i+1][2], v)
			case 2:
				res[i+1][1] = max(res[i+1][1], v)
				res[i+1][2] = max(res[i+1][2], v)
			}

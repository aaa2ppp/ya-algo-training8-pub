...
type solveFunc func(s string, words []string) []string
...
func solve(s string, words []string) []string {
	...
	if idxs[n] == -1 {
		panic(fmt.Sprintln("logical error: last index cnnot be -1"))
	}
	...
	for j := n; j > 0; {
		i := idxs[j]
		if !(0 <= i && i < j) {
			panic(fmt.Sprintf("%d: logical error: i must in [0; %d), got %d", j, j, i))
		}
		sentence = append(sentence, s[i:j])
		j = i
	}

	slices.Reverse(sentence)
	return sentence
}

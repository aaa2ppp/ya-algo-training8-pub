...
type solveFunc func(river string) int

func solve(river string) int {
	const (
		L = 0
		R = 1
	)
	...
	count := make([][2]int, n+1)
	for i := range count {
		count[i] = [2]int{math.MaxInt, math.MaxInt}
	}
	count[0] = [2]int{0, 1}

	for i, c := range []byte(river) {
		// (i, L) -> (i, R)
		count[i][R] = min(count[i][R], count[i][L]+1)
		// (i, R) -> (i, L)
		count[i][L] = min(count[i][L], count[i][R]+1)

		// (i, L) -> (i+1, L)
		// (i, R) -> (i+1, R)
		switch c {
		case 'L':
			count[i+1][L] = min(count[i+1][L], count[i][L]+1)
			count[i+1][R] = min(count[i+1][R], count[i][R])

		case 'R':
			count[i+1][L] = min(count[i+1][L], count[i][L])
			count[i+1][R] = min(count[i+1][R], count[i][R]+1)
		...
		default:
			panic(fmt.Sprintf("%d: unknown letter '%c'", i, c))
		}
	}

	// Важно! (n, L) -> (n, R)
	count[n][R] = min(count[n][R], count[n][L]+1)
...
func run(in io.Reader, out io.Writer, solve solveFunc) {
	...
	river, err := br.ReadString('\n')
	if err != nil && err != io.EOF {
		panic(err)
	}
	river = strings.TrimSpace(river)

...
func solve(k int, a []int) []int {
	...
	for i := 0; i+k <= n; i++ {
		...
		// TODO: здесь N -> общая сложность N^2
		// Чтобы использовать результаты предыдущего подсчета,
		// нужно городить минимум в окне...
		// И так должно зайти 1000*1000 = 1_000_000
		var (
			total   = 0
			minimum = math.MaxInt
		)
		...
		security := res[i][0] + total * minimum

		if res[i+k][0] < security {
			res[i+k] = [2]int{security, i}
		}

		if maximum[0] < security {
			maximum = [2]int{security, i}
		}
	}
	...
	for prev >= 0 {
		ans = append(ans, prev+1) // to 1-indexing
		prev = res[prev][1]
	}

...
type solveFunc func(t []string) bool
...
func solve(t []string) bool {
	...
	for _, x := range []byte{'X', 'O'} {
		...
		for k := 0; k < n; k++ {
			count = 0
			for i, j := k, 0; i < n && j < m; i, j = i+1, j+1 {
				if t[i][j] == x {
					count++
					if count == 5 {
						return true
		...
		for k := 0; k < m; k++ {
			count = 0
			for i, j := 0, k; i < n && j < m; i, j = i+1, j+1 {
				if t[i][j] == x {
					count++
					if count == 5 {
						return true
		...
		for k := 0; k < n; k++ {
			count = 0
			for i, j := k, 0; i >= 0 && j < m; i, j = i-1, j+1 {
				if t[i][j] == x {
					count++
					if count == 5 {
						return true
		...
		for k := 0; k < m; k++ {
			count = 0
			for i, j := 0, k; i < n && j >= 0; i, j = i+1, j-1 {
				if t[i][j] == x {
					count++
					if count == 5 {
						return true
...
func run(in io.Reader, out io.Writer, solve solveFunc) {
	...
	for i := 0; i < n; i++ {
		...
		if len(r) != m {
			panic(fmt.Errorf("len(r) = %v, want %v", len(r), m))
		}
		t = append(t, r)
	}

...
type solveFunc func(x, y, f, g int) int

func solve(x, y, f, g int) int {
	dx := abs(x - f)
	dy := abs(y - g)
	s := dx + dy
	...
	// в начале движения, можем двигаться в любую сторону без маневра
	s--

	// чтобы проехать один квартал по прямой нужно сделать три маневра
	if dx == 0 || dy == 0 {
		// нет ни одного поворота
		s *= 3
	} else {
		// если есть поворот, он всегда может быть левым;
		// после поворота на лево можно проехать один квартал не "извращаясь"
		s = (s-1)*3 + 1
	}

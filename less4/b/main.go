...
type solveFunc func(taxes []tax, q []int) []int

func solve(taxes []tax, q []int) []int {
	n, m := len(taxes), len(q)
	ans := make([]int, m)
...
func run(in io.Reader, out io.Writer, solve solveFunc) {
	...
	taxes := make([]tax, 0, n)
	for range n {
		b, t, err := scanTwoInt(sc)
		if err != nil {
			panic(err)
		}
		taxes = append(taxes, tax{b, t})
	}
	...
	q := make([]int, m)
	if _, err := scanInts(sc, q); err != nil {
		panic(err)
	}

...
type solveFunc func(s, a []int) (int, int)
...
func solve(s, a []int) (int, int) {
	n := len(s)

	events := make([]Event, 1, n+1) // first is zero
	...
	for i := range n {
		events = append(events, Event{s[i], a[i]})
		rightA += a[i]
		rightCost += a[i] * s[i]
	}
	...
	for i, e := range events {
		if debugEnable {
			log.Println(e.s, leftCost+rightCost)
		}
	...
	panic("logical error: we should have returned earlier (inside the loop)")
}
...
func run(in io.Reader, out io.Writer, solve solveFunc) {
	...
	if _, err := scanInts(sc, s); err != nil {
		panic(err)
	}
	...
	count, cost := solve(s, a)
	writeInts(bw, []int{count, cost})
}

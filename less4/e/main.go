...
type solveFunc func(a []int, t []trip, k int) int

// solve: a - 1-indexing, a[0] not used
func solve(a []int, t []trip, k int) int {
	n := len(a) - 1
	...
	sumT := make([]int, n+2) // нам технически понадобиться n+1
	total := 0
	for i := range t {
		l, r := t[i].l, t[i].r
		sumT[l]++
		sumT[r+1]-- // вот здесь нужен n+1
		total += sumA[r] - sumA[l-1]
	}

	sumT = sumT[:n+1] // обрезаем - n+1 больше не нужен
	...
	slices.SortFunc(o, func(i, j int) int {
		return sumT[j] - sumT[i]
	})
...
func run(in io.Reader, out io.Writer, solve solveFunc) {
	...
	a := make([]int, n+1) // 1-indexing, a[0] not used

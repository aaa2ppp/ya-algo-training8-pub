...
type solveFunc func(front, back []Trip) int
...
const (
	...
	// порядок важен! прибытие учитываем всегда раньше отправления
	Arrival
	Departure
)
...
func solve(front, back []Trip) int {
	n, m := len(front), len(back)
	events := make([]Event, 0, (n+m)*2)

	var a, b int // points

	for _, trip := range front {
		events = append(events,
			Event{trip.Departure, Departure, &a},
			Event{trip.Arrival, Arrival, &b},
		)
	}

	for _, trip := range back {
		events = append(events,
			Event{trip.Departure, Departure, &b},
			Event{trip.Arrival, Arrival, &a},
		)
	}

	slices.SortFunc(events, func(a, b Event) int {
		return strings.Compare(a.Time, b.Time)*4 + int(a.Type-b.Type)
	})
...
func run(in io.Reader, out io.Writer, solve solveFunc) {
	...
	for range n {
		...
		front = append(front, Trip{Departure: s[:5], Arrival: s[6:]})
	}

	if debugEnable {
		log.Print("front:", front)
	}
	...
	for range m {
		...
		back = append(back, Trip{Departure: s[:5], Arrival: s[6:]})
	}
...
func _parseInt[X Int](b []byte) (X, error) {
	if ^X(0) < 0 {
		v, err := strconv.ParseInt(unsafeString(b), 10, int(unsafe.Sizeof(X(1)))<<3)
		return X(v), err
	} else {
		v, err := strconv.ParseUint(unsafeString(b), 10, int(unsafe.Sizeof(X(1)))<<3)
		return X(v), err
	}
}

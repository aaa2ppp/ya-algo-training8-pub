...
type solveFunc func(d int32, trees []Tree) int32

type Tree struct {
	X, Y int32
}

// solve считает количество различных отрезков между точками заданными trees,
// квадрат длины которых в точности равен d
func solve(d int32, trees []Tree) int32 {
	n := len(trees)

	offsets := calcOffsets(d)
	if len(offsets) == 0 {
		return 0
	}

	forest := make(map[[2]int32]int32, n)
	for _, t := range trees {
		forest[[2]int32{t.X, t.Y}]++
	}

	count := int32(0)
	for _, t := range trees {
		for _, o := range offsets {
			if n, ok := forest[[2]int32{t.X + o[0], t.Y + o[1]}]; ok {
				count += n
			}
		}
	}
...
// calcOffsets возвращает все возможные смещения, сумма квадратов которых в точности равна d.
func calcOffsets(d int32) [][2]int32 {
	qs := calcSquareSums(d)
	offsets := make([][2]int32, 0, len(qs)*8)

	for _, s := range qs {
		switch s[0] {
		case 0:
			offsets = append(offsets, [2]int32{0, s[1]}, [2]int32{0, -s[1]}, [2]int32{s[1], 0}, [2]int32{-s[1], 0})
		case s[1]:
			offsets = append(offsets, [2]int32{s[0], s[0]}, [2]int32{s[0], -s[0]}, [2]int32{-s[0], -s[0]}, [2]int32{-s[0], s[0]})
		default:
			offsets = append(offsets, [2]int32{s[0], s[1]}, [2]int32{s[0], -s[1]}, [2]int32{-s[0], -s[1]}, [2]int32{-s[0], s[1]})
			offsets = append(offsets, [2]int32{s[1], s[0]}, [2]int32{s[1], -s[0]}, [2]int32{-s[1], -s[0]}, [2]int32{-s[1], s[0]})
		}
	}
	return offsets
}

// calcSquares возвращает слайс всех квадратов (начиная с 0) меньших либо равных d.
// Значения слайса упорядочены по возрастанию.
func calcSquares(d int32) []int32 {
	q := make([]int32, 0)
	for i := 0; i*i <= int(d); i++ {
		q = append(q, int32(i*i))
	}
	return q
}

// calcSquareSumsSlow возвращает слайс всех пар сумма квадратов которых в точности равна d.
// Гарантируется, что первый элемент пары не больше второго.
func calcSquareSumsSlow(d int32) [][2]int32 {
	q := calcSquares(d)
	...
	for i := 0; i < len(q); i++ {
		for j := i; j < len(q); j++ {
			if q[i]+q[j] == d {
				qs = append(qs, [2]int32{int32(i), int32(j)})
			}
		}
	}
...
// calcSquareSums возвращает слайс всех пар сумма квадратов которых в точности равна d.
// Гарантируется, что первый элемент пары не больше второго.
func calcSquareSums(d int32) [][2]int32 {
	q := calcSquares(d)
...
func run(in io.Reader, out io.Writer, solve solveFunc) {
	...
	trees := make([]Tree, 0, n)
	for range n {
		x, y, err := scanTwoIntX[int32](sc)
		if err != nil && err != io.EOF {
			panic(err)
		}
		trees = append(trees, Tree{x, y})
	}

...
type solveFunc func(a []int) (int, int, int)

func solve(a []int) (int, int, int) {
	// судя по всему, должны съесть по крайней мере по одному сырку
	l, r := 0, len(a)-1
	d := a[l] - a[r]

	minimum, lMin, rMin := abs(d), l, r
	...
	for l+1 < r {
		...
		if abs(d) < minimum { // обжорству бой!
			minimum, lMin, rMin = abs(d), l, r
		}
	}

	return minimum, lMin, rMin
}
...
func run(in io.Reader, out io.Writer, solve solveFunc) {
	...
	d, l, r := solve(a)
	writeInts(bw, []int{d, l + 1, r + 1}) // to 1-indexing
}

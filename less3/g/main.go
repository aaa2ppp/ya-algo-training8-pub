...
func solve1(a, b []int) int {
	...
	a2 := make([][2]int, maxA+1)
	b2 := make([][2]int, maxB+1)
	...
	// TODO: O(max(ai) * max(bj))
	for ai, ae := range a2 {
		for bj, be := range b2 {
			ans += (ae[0]*be[1] - be[0]*ae[1]) * abs(ai-bj)
		}
	}
...
func run(in io.Reader, out io.Writer, solve solveFunc) {
	...
	b := make([]int, m+1)
	if _, err := scanInts(sc, b[1:]); err != nil {
		panic(err)
	}

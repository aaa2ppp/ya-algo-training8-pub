...
type solveFunc func(p int, c []int) (int, int)

func solve(p int, c []int) (int, int) {
	n := len(c)
	o := make([]int, n)
	for i := range o {
		o[i] = i
	...
	slices.SortFunc(o, func(a, b int) int {
		return c[a] - c[b]
	})
	...
	for i := range o {
		...
		j, _ := slices.BinarySearchFunc(o, p*m, func(x, t int) int {
			return c[x] - t
		})

		if j == len(o) {
			j--
		} else if j > 0 && p*m-c[o[j-1]] < c[o[j]]-p*m {
			j--
		}

		f := math.Abs(float64(c[o[j]]-p*m) / float64(m))
		if f < fmin {
			fmin = f
			a, b = o[j]+1, o[i]+1
		}
...
func run(in io.Reader, out io.Writer, solve solveFunc) {
	...
	n, p, err := scanTwoInt(sc)
	if err != nil {
		panic(err)
	}

	c := make([]int, n)
	if _, err := scanInts(sc, c); err != nil {
		panic(err)
	}

	a, b := solve(p, c)
	writeInts(bw, []int{a, b})
}

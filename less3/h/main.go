...
type solveFunc func(a []int, e [][2]int) int
...
// solve. 'a' содержит значения узлов дерева (количество жителей) с 1-индексацией; a[0] не используется.
func solve(a []int, edges [][2]int) int {
	n := len(a)

	tree := make([]node, n)
	for _, e := range edges {
		v, u := e[0], e[1]
		tree[v].neigs = append(tree[v].neigs, neig{node: u})
		tree[u].neigs = append(tree[u].neigs, neig{node: v})
	}

	// спрашиваем соседий сколько с их стороны придет
	var dfs1 func(node, prev int) int

	dfs1 = func(node, prev int) int {
		tree[node].total = a[node]
		for i, next := range tree[node].neigs {
			if next.node == prev {
				continue
			}
			s := dfs1(next.node, node)
			tree[node].total += s
			tree[node].neigs[i].sum = s
		}
		return tree[node].total
	}

	// рассказываем соседям сколько с нашей стороны придет
	var dfs2 func(node, prev int, s int)

	dfs2 = func(node, prev int, s int) {
		tree[node].total += s
		for i, next := range tree[node].neigs {
			if next.node == prev {
				tree[node].neigs[i].sum = s
			...
			dfs2(next.node, node, tree[node].total-next.sum)
		}
	}

	dfs1(1, 0)
	if debugEnable {
		log.Println("tree1:", tree)
	}

	dfs2(1, 0, 0)
	if debugEnable {
		log.Println("tree2:", tree)
	}
	...
	for node := 1; node < n; node++ {
		maximum := a[node]
		for _, neig := range tree[node].neigs {
			maximum = max(maximum, neig.sum)
		}
		if maximum < minimax {
			minimax = maximum
			minimaxNode = node
...
func run(in io.Reader, out io.Writer, solve solveFunc) {
	...
	a := make([]int, n+1)
	if _, err := scanInts(sc, a[1:]); err != nil {
		panic(err)
	}

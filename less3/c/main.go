...
type solveFunc func(w, h int, words []word) float64

func format(wf, hf float64, words []word, k float64) bool {
	var (
		n          = len(words)
		lineWidth  = float64(words[0].a) * k
		lineHeight = float64(words[0].b) * k
		pageHeigth = float64(0)
	)

	for i := 1; i < n; i++ {
		w := words[i]
		ww, wh := float64(w.a)*k, float64(w.b)*k

		if w.b == words[i-1].b && lineWidth+ww < wf {
			lineWidth += ww
			lineHeight = math.Max(lineHeight, wh)
			continue
		}

		pageHeigth += lineHeight
	...
	return pageHeigth <= hf
}

func solve(w, h int, words []word) float64 {
	if debugEnable {
		log.Println("words:", words)
	}
	var sumA, sumB, maxA, maxB int
	...
	minK := math.Min(wf/float64(sumA), hf/float64(sumB))
	maxK := math.Min(wf/float64(maxA), hf/float64(maxB))

	if debugEnable {
		log.Printf("k in [%g, %g]", minK, maxK)
	}
	...
	for {
		...
		if (maxK-minK)/math.Max(mid, 1) < 1e-6 {
			return mid
		}
		if format(wf, hf, words, mid) {
			minK = mid
		} else {
			maxK = mid
		}

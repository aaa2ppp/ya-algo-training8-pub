...
type solveFunc func(p []int, q [][2]int) []byte

func solve(parent []int, queries [][2]int) []byte {
	n, m := len(parent), len(queries)
	ans := make([]byte, m)
	...
	// строим мапу вопросов, для каждого узла подвешиваем список вопросов по поводу его предка
	queryMap := make(map[int][]query, m)
	for i, q := range queries {
		queryMap[q[1]] = append(queryMap[q[1]], query{i, q[0]})
	}

	// строим дерево (деревья)
	tree := make([][]int, n)
	for node := 1; node < n; node++ {
		tree[parent[node]] = append(tree[parent[node]], node)
	}

	// текущее множество предков
	parents := make(map[int]struct{}, n)

	var dfs func(int)

	// обходим дерево и отвечаем на вопросы
	dfs = func(node int) {
		if nodeQueries, ok := queryMap[node]; ok {
			for _, query := range nodeQueries {
				if _, ok := parents[query.parent]; ok {
					ans[query.idx] = 1
				}
			}
		}
	...
	// ищем корень для каждого дерева и начинаем обход
	for node := 1; node < n; node++ {
		if parent[node] == 0 {
			dfs(node)
		}
...
func slowSolve(p []int, q [][2]int) []byte {
	n := len(p)

	t := make([][]int, n)
	for i := 1; i < n; i++ {
		t[p[i]] = append(t[p[i]], i)
	}

	var dfs func(i int, r, h int)

	h := make([]int, n)
	r := make([]int, n)
	dfs = func(i int, rr, hh int) {
		r[i] = rr 
		h[i] = hh
		for _, j := range t[i] {
			dfs(j, rr, hh+1)
...
func run(in io.Reader, out io.Writer, solve solveFunc) {
	...
	p := make([]int, n+1)
	if _, err := scanInts(sc, p[1:]); err != nil {
		panic(err)
	}
...
var (
	...
	_, useSlowSolve = os.LookupEnv("SLOW")
)
